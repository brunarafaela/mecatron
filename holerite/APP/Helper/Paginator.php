...
class Paginator {
	...
	public function go($page = false, $totalPages = false, $link = false, $perpage = 10) {
		...
		if ($page && $totalPages && $link) {
			...
			if ($page > 1) {
				if ($page > 2) {
					$return .= '<a class="btn" href="'.$link.(1).'">Primeira</a>';
					$return .= '<a class="btn" href="'.$link.($page - 1).'">Anterior</a>';
				} else {
					$return .= '<a class="btn" href="'.$link.($page - 1).'">Anterior</a>';
				}
			}
			...
			if ($page < $totalPages) {
				if (($page + 1) == $totalPages) {
					$return .= '<a class="btn" href="'.$link.($totalPages).'">Ultima</a>';	
				} else {
					$return .= '<a class="btn" href="'.$link.($page + 1).'">Proxima</a>';
					$return .= '<a class="btn" href="'.$link.($totalPages).'">Ultima</a>';	
				}
			}
